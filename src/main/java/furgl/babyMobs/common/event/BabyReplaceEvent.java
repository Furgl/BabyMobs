...
public class BabyReplaceEvent 
{
	...
	public void onEvent(LivingEvent.LivingUpdateEvent event)
	{
		...
		if (!event.getEntity().world.isRemote && event.getEntity().ticksExisted == 1 && event.getEntity() instanceof EntityHorse && (((EntityHorse)event.getEntity()).getType() == HorseType.ZOMBIE || ((EntityHorse)event.getEntity()).getType() == HorseType.SKELETON))
		{
		...
		if (!event.getEntity().world.isRemote && event.getEntity().ticksExisted == 1 && !event.getEntity().getEntityData().hasKey("Baby Mobs: Checked")) //if server side & newly spawned
		{
			...
			if (event.getEntity().getClass() == EntityZombie.class)
			{
				...
				if (rand.nextInt(100) < Config.babyZombieRate || event.getEntity().getRidingEntity() instanceof EntityHorse && ((EntityHorse)event.getEntity().getRidingEntity()).isChild())
				{
					...
					if (!(entity.getRidingEntity() instanceof EntityHorse)) {
						EntityLiving entityToSpawn = new EntityZombieChicken(entity.world);
						entityToSpawn.setLocationAndAngles(entity.posX, entity.posY, entity.posZ,  entity.rotationYaw, 0.0F);
						entityToSpawn.onInitialSpawn(entity.world.getDifficultyForLocation(entity.getPosition()), (IEntityLivingData)null);
			...
			{
				EntityLiving entityToSpawn = new EntityZombieChicken(event.getEntity().world);
				...
				entityToSpawn.onInitialSpawn(event.getEntity().world.getDifficultyForLocation(event.getEntity().getPosition()), (IEntityLivingData)null);
				((EntityZombieChicken) entityToSpawn).setChickenJockey(true);
				event.getEntity().world.spawnEntity(entityToSpawn);
			...
			else if (event.getEntity().getClass() == EntityPigZombie.class)
			{
				...
				if (rand.nextInt(100) < Config.babyPigZombieRate)
				{
					...
					event.getEntity().setDead();
					EntityLiving entityToSpawn = new EntityZombiePig(entity.world);//(EntityLiving) EntityList.createEntityByName("babymobs.zombiePig", entity.world);
					entityToSpawn.setLocationAndAngles(entity.posX, entity.posY, entity.posZ,  MathHelper.wrapDegrees(entity.world.rand.nextFloat() * 360.0F), 0.5F);
					entity.world.spawnEntity(entityToSpawn);
					entity.startRiding(entityToSpawn);
					entityToSpawn.playLivingSound();
				}
			...
			{
				EntityLiving entityToSpawn = new EntityZombiePig(event.getEntity().world);//(EntityLiving) EntityList.createEntityByName("babymobs.zombiePig", event.getEntity().world);
				entityToSpawn.setLocationAndAngles(event.getEntity().posX, event.getEntity().posY, event.getEntity().posZ,  MathHelper.wrapDegrees(event.getEntity().world.rand.nextFloat() * 360.0F), 0.5F);
				event.getEntity().world.spawnEntity(entityToSpawn);
				event.getEntity().startRiding(entityToSpawn);
				entityToSpawn.playLivingSound();
			}
			...
			{
				if (event.getEntity().world.provider.getDimensionType() == DimensionType.THE_END/*getBiomeGenForCoords(event.getEntity().getPosition()) instanceof BiomeEnd*/ && rand.nextInt(100) < Config.babyEndermanEndRate)
				{
					this.spawnEntity(EntityBabyEnderman.class, event.getEntity());
					event.getEntity().setDead();
				}
				else if (event.getEntity().world.provider.getDimensionType() != DimensionType.THE_END && rand.nextInt(100) < Config.babyEndermanRate)
				{
					this.spawnEntity(EntityBabyEnderman.class, event.getEntity());
					event.getEntity().setDead();
				}
			...
			else if (event.getEntity().getClass() == EntityWitch.class)
			{
				if (rand.nextInt(100) < Config.babyWitchRate)
				{
					...
					EntityLiving entityToSpawn = new EntityBabyOcelot(event.getEntity().world);//(EntityLiving) EntityList.createEntityByName("babymobs.babyOcelot", event.getEntity().world);
					entityToSpawn.setLocationAndAngles(event.getEntity().posX, event.getEntity().posY, event.getEntity().posZ,  MathHelper.wrapDegrees(event.getEntity().world.rand.nextFloat() * 360.0F), 0.5F);
					event.getEntity().world.spawnEntity(entityToSpawn);
					entityToSpawn.playLivingSound();
					event.getEntity().setDead();
