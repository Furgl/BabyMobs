...
public class EntityBabySkeleton extends EntitySkeleton
{
	...
	public void onDeath(DamageSource cause) //first achievement
	{
		...
		if (!this.world.isRemote)
		{
			...
			ItemStack stack = new ItemStack(Items.TIPPED_ARROW, this.world.rand.nextInt(3)+1);
	...
	public boolean attackEntityAsMob(Entity entity)
	{
		...
		{
			if (!this.world.isRemote && entity instanceof EntityLivingBase && !(entity instanceof EntityPlayer && ((EntityPlayer)entity).capabilities.isCreativeMode))
	...
	public void setEffectType() { 
		if (this.dataManager.get(POTION_EFFECT).intValue() == 0 && !this.world.isRemote)
		{
	...
	public void onLivingUpdate()
	{
		...
		if (Config.useSpecialAbilities)
		{
			if (this.effectType == 0)
				this.setEffectType();
			if (this.world.isRemote && this.effectType != 0 && this.red == -1)
				this.setColors();
			if (this.world.isRemote && this.red != -1)
			{
				if (this.ticksExisted == 3)
					BabyMobs.proxy.spawnEntitySkeletonEffectFX(this.world, this, (float) this.red, (float) this.green, (float) this.blue);
				this.world.spawnParticle(EnumParticleTypes.SPELL_MOB, this.posX + (this.rand.nextDouble() - 0.5D) * this.width, this.posY + this.rand.nextDouble() * this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, this.red, this.green, this.blue, new int[0]);
			}
		}
		//end
	...
	public void attackEntityWithRangedAttack(EntityLivingBase target, float p_82196_2_)
	{
		...
		if (Config.useSpecialAbilities && !this.world.isRemote)
