...
//			((EntityPlayer)cause.getEntity()).addStat(Achievements.achievementWhyAreTheySoStrong);
	...
	public void onUpdate()
	{
		...
		if (this.f > f1)
			this.f = MathHelper.clamp(this.f - 0.05F, f1, 1.0F);
		else if (this.f < f1)
			this.f = MathHelper.clamp(this.f + 0.05F, 0.0F, f1);
	...
	class AIAttack extends EntityAIBase
    {
        ...
        public boolean shouldExecute()
        {
            ...
            return entitylivingbase != null && entitylivingbase.isEntityAlive() ? EntityBabyShulker.this.world.getDifficulty() != EnumDifficulty.PEACEFUL : false;
        }
        ...
        {
            if (EntityBabyShulker.this.world.getDifficulty() != EnumDifficulty.PEACEFUL)
            {
                --this.field_188520_b;
                EntityLivingBase entitylivingbase = EntityBabyShulker.this.getAttackTarget();
                ...
                if (d0 < 400.0D)
                {
                    if (this.field_188520_b <= 0)
                    {
                        ...
                        EntityBabyShulkerBullet entityshulkerbullet = new EntityBabyShulkerBullet(EntityBabyShulker.this.world, EntityBabyShulker.this, entitylivingbase, EntityBabyShulker.this.getAttachmentFacing().getAxis());
                        EntityBabyShulker.this.world.spawnEntity(entityshulkerbullet);
