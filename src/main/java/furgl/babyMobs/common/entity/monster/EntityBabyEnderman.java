...
public class EntityBabyEnderman extends EntityMob
{
	...
	public EntityBabyEnderman(World p_i1734_1_)
	{
		super(p_i1734_1_);this.setSize(0.6F, 1.6F);
	...
	public EntityPlayer getTargetedEntity()
	{
		...
		{
			//if fake player or creative mode
			if (this.entityToAttack instanceof FakePlayer || (this.entityToAttack instanceof EntityPlayer && ((EntityPlayer) this.entityToAttack).capabilities.isCreativeMode))
				return null;
			return this.entityToAttack instanceof EntityPlayer ? (EntityPlayer) this.entityToAttack : null;
		}
	}
	//end
	...
	public void onLivingUpdate()
	{
		...
		if (Config.useSpecialAbilities)
		{
			if (!this.worldObj.isRemote)
			{
				...
				{
					if(this.entityToAttack instanceof EntityPlayer && this.dataWatcher.getWatchableObjectInt(19) != 0) 
					{
						this.entityToAttack.attackEntityFrom(DamageSource.magic, 1F);
					}
					if (this.entityToAttack instanceof EntityPlayer && this.shouldAttackPlayer((EntityPlayer) this.entityToAttack) == true && this.canEntityBeSeen(this.entityToAttack) && this.getHealth() > 0)
					{
						this.dataWatcher.updateObject(19, Integer.valueOf(this.entityToAttack.getEntityId()));
						laserDelay = 40;
				...
				{
					if (laserDelay > 0 && laserDelay % 5 == 0 && this.entityToAttack != null) 
					{
						if (this.entityToAttack instanceof EntityPlayer && !(this.entityToAttack instanceof FakePlayer))
							((EntityPlayer)this.entityToAttack).triggerAchievement(Achievements.achievementLaserTag);
						this.entityToAttack.attackEntityFrom(DamageSource.magic, 1F);
					}
					laserDelay--;
					this.dataWatcher.updateObject(31, Integer.valueOf(laserDelay));
		...
		if (!this.worldObj.isRemote && this.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing"))
		{
			...
			if (this.getCarriedBlock().getMaterial() == Material.air)
			{
				if (this.rand.nextInt(20) == 0)
				{
					...
					if (EntityBabyEnderman.getCarriable(block))
					{
		...
		if (Config.useSpecialAbilities)
		{
			if (this.getHealth() == 0 || (!this.worldObj.isRemote && this.entityToAttack instanceof EntityPlayer && !this.canEntityBeSeen(this.entityToAttack)))
			{
