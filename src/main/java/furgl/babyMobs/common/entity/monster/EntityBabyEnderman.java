...
//			((EntityPlayer)cause.getEntity()).addStat(Achievements.achievementWhyAreTheySoStrong);
	...
	public EntityPlayer getTargetedEntity()
	{
		...
		{
			Entity entity = this.world.getEntityByID(Math.round(this.dataManager.get(EntityBabyEnderman.ENTITY_ID)));
	...
	public void onLivingUpdate()
	{
		...
		if (Config.useSpecialAbilities)
		{
			if (this.dataManager.get(EntityBabyEnderman.ENTITY_ID) != 0)
			{
				if (this.getHealth() == 0 || (!this.world.isRemote && this.getAttackTarget() instanceof EntityPlayer && !(this.getAttackTarget() instanceof FakePlayer) && !this.canEntityBeSeen(this.getAttackTarget())))
				{
    ...
    protected boolean isMovementBlocked() {
        return this.getHealth() <= 0.0F || this.dataManager.get(EntityBabyEnderman.ENTITY_ID) != 0;
    }
}